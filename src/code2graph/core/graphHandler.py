...
from .json2graph import json2RDF
...
class graphHandler(graphMETA):
    ...
    def convertJson2RDF(self, jsonGraph=None):
        ...
        self._RDF, self._sRDF = json2RDF(self._jsonGraph, self._logdir)
        return
    ...
    def convertGraphDef2Json(self, graphDef=None):
        if(graphDef == None):
            self._jsonGraph = json.loads(
                json_format.MessageToJson(self._graphDef))
        else:
        ...
        for path, dir_list, file_list in os.walk(path_to_dir):
            for dir in dir_list:
                shutil.rmtree(path=os.path.join(
                    path_to_dir, dir), ignore_errors=False)
