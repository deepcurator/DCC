...
class CallGraphVisitor(ast.NodeVisitor):
    ...
    def visit_Module(self, node):
        self.logger.debug("Module %s" % (self.module_name))
    ...
    def visit_FunctionDef(self, node):
        ...
        if self_name is not None:
            ...
            self.logger.debug('Method def: setting self name "%s" to %s' % (self_name, class_node))
    ...
    def check_value_type(self, defaultval_node):
        if isinstance(defaultval_node, ast.Constant):
            return defaultval_node.value
        elif isinstance(defaultval_node, (Str, Bytes)):
            return defaultval_node.s
        elif isinstance(defaultval_node, Num):
            return defaultval_node.n
        elif isinstance(defaultval_node, Tuple):
            return tuple(map(check_value_type, defaultval_node.elts))
        elif isinstance(defaultval_node, List):
            return list(map(check_value_type, defaultval_node.elts))
        elif isinstance(defaultval_node, Set):
            return set(map(check_value_type, defaultval_node.elts))
        elif isinstance(defaultval_node, Dict):
            return dict((check_value_type(k), check_value_type(v)) for k, v
                        in zip(defaultval_node.keys, defaultval_node.values))
        elif isinstance(defaultval_node, NameConstant):
            return defaultval_node.value
        elif isinstance(defaultval_node, UnaryOp) and isinstance(defaultval_node.op, (UAdd, USub)):
            operand = check_value_type(defaultval_node.operand)
            if isinstance(operand, _NUM_TYPES):
                if isinstance(defaultval_node.op, UAdd):
                    return + operand
                else:
                    return - operand
        elif isinstance(defaultval_node, BinOp) and isinstance(defaultval_node.op, (Add, Sub)):
            left = check_value_type(defaultval_node.left)
            right = check_value_type(defaultval_node.right)
            if isinstance(left, _NUM_TYPES) and isinstance(right, _NUM_TYPES):
                if isinstance(defaultval_node.op, Add):
                    return left + right
                else:
                    return left - right
        raise ValueError('malformed node or string: ' + repr(defaultval_node))

    def generate_args_nodes(self, ast_args, inner_ns):
        self.logger.debug("Argument %s", (ast_args.args))
        """Capture which names correspond to function args.
        ...
        # self.logger.debug('nonsense_node %s', nonsense_node)
        ...
        args2 = []
        n_plain = len(ast_args.args) - len(ast_args.defaults)
        for i in range(len(ast_args.args)):
            to_node = self.get_node(ns, ast_args.args[i].arg, flavor=Flavor.ARGUMENT)
            if self.add_argument_edge(from_node, to_node):
                self.logger.info("Def from %s to Argument %s" % (from_node, to_node))
            self.set_value(ast_args.args[i].arg, to_node)
			
            self.name_stack.append(ast_args.args[i].arg)
            inner_ns_arg = self.get_node_of_current_namespace().get_name()
            # self.scope_stack.append(self.scopes[inner_ns_arg])
            self.context_stack.append("Argument %s" % (ast_args.args[i].arg))
									
            if i < n_plain:
                self.logger.info("No Default value")
            else:
                # try:
                self.logger.info('Default Value %s=%s' % (ast_args.args[i].arg, ast_args.defaults[i - n_plain]))
                defaultval_node = ast_args.defaults[i - n_plain]
                value = None
                if isinstance(defaultval_node, ast.Constant):
                    value = defaultval_node.value
                elif isinstance(defaultval_node, (ast.Str, ast.Bytes)):
                    value =  defaultval_node.s
                elif isinstance(defaultval_node, ast.Num):
                    value = defaultval_node.n
                elif isinstance(defaultval_node, ast.NameConstant):
                    value = defaultval_node.value
                self.logger.info('default value %s' % (value))              
                self.check_default(value, inner_ns_arg)
        ...
        # if ast_args.vararg is not None:  # *args if present
            # sc.defs[ast_args.vararg] = self.get_node(inner_ns, a.arg, flavor=Flavor.ARGUMENT)
        # for a in ast_args.kwonlyargs:    # any after *args or *
            # sc.defs[a.arg] = self.get_node(inner_ns, a.arg, flavor=Flavor.ARGUMENT)
        # if ast_args.kwarg is not None:  # **kwargs if present
            # sc.defs[ast_args.kwarg] = self.get_node(inner_ns, a.arg, flavor=Flavor.ARGUMENT)
					
    def check_default(self, default_value, inner_ns_arg):
        from_node = self.get_node_of_current_namespace()
        ns = from_node.get_name()
				
        to_node = self.get_node(ns, default_value, flavor=Flavor.DEFAULTVALUE)
        if self.add_value_edge(from_node, to_node):
            self.logger.info("Arg from %s to Value %s" % (from_node, to_node))
        self.set_value(default_value, to_node)
			
        self.name_stack.append(default_value)
        inner_ns_val = self.get_node_of_current_namespace().get_name()
		# self.scope_stack.append(self.scopes[inner_ns_arg])
        self.context_stack.append("Argument %s" % (default_value))
    ...
    def analyze_arguments(self, ast_args):
        ...
        if ast_args.defaults:
            ...
            self.logger.info("Defaults %s" % (ast_args.defaults))
            for tgt, val in zip(ast_args.args[-n:], ast_args.defaults):
                ...
                self.logger.info("target %s to value %s" % (targets, values))				
    ...
    def visit_ImportFrom(self, node):
        ...
        if tgt_name in self.module_names:
            ...
            mod_name = tgt_name
            self.name_stack.append(mod_name)
            # self.scope_stack.append(self.scopes[tgt_name])
            self.context_stack.append("Module %s" % (mod_name))
        ...
        for import_item in node.names:
            ...
            if tgt_id in self.module_names:
                m_name = self.module_names[tgt_id]
            else:
                m_name = tgt_id
                self.name_stack.append(tgt_id)
                # self.scope_stack.append(self.scopes[tgt_name])
                self.context_stack.append("Module %s" % (tgt_id))
    ...
    def analyze_functiondef(self, ast_node):
        ...
        for deco in ast_node.decorator_list:
            ...
            self.logger.info("Name is %s" % (deco_node.name))
    ...
    def analyze_binding(self, targets, values):
        ...
        if len(targets) == len(values):  # handle correctly the most common trivial case "a1,a2,... = b1,b2,..."
            ...
            for value in values:
                self.logger.debug("In else Value %s" % (value))
            ...
            for tgt in targets:    # LHS, name in a store context
                self.logger.debug("In else Target %s" % (tgt))
                self.visit(tgt)
            self.last_value = None
    ...
    def add_argument_edge(self, from_node, to_node):
        """Add a argument edge in the graph between two nodes.
        N.B. This will mark both nodes as defined."""

        if from_node not in self.argument_edges:
            self.argument_edges[from_node] = set()
        if to_node in self.argument_edges[from_node]:
            return False
        self.argument_edges[from_node].add(to_node)
        from_node.defined = True
        to_node.defined = True
        return True
		
    def add_value_edge(self, from_node, to_node):
        """Add a default value edge in the graph between two nodes.
        N.B. This will mark both nodes as defined."""

        if from_node not in self.default_value_edges:
            self.default_value_edges[from_node] = set()
        if to_node in self.default_value_edges[from_node]:
            return False
        self.default_value_edges[from_node].add(to_node)
        from_node.defined = True
        to_node.defined = True
        return True
    ...
    def add_uses_edge(self, from_node, to_node):
        ...
        if to_node.namespace is not None:
            self.logger.debug('Removing Wildcard: %s , %s', to_node.name, to_node.namespace)
