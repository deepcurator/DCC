...
class VisualEdge(object):
    ...
    flavor is meant to be 'uses', 'defines' or 'argument' or 'val'
    """
...
class VisualGraph(object):
    ...
    def from_visitor(cls, visitor, options=None, logger=None):
        ...
        draw_arguments = options.get('draw_arguments', False)
        draw_default_values = options.get('draw_default_values', False)
        ...
        if draw_arguments:
            color = '#ff00ff'
            for n in visitor.argument_edges:
                if n.defined:
                    for n2 in visitor.argument_edges[n]:
                        if n2.defined:
                            root_graph.edges.append(
                                    VisualEdge(
                                        nodes_dict[n],
        ...
        if draw_default_values:
            color = '#ffff00'
            for n in visitor.default_value_edges:
                if n.defined:
                    for n2 in visitor.default_value_edges[n]:
                        if n2.defined:
                            root_graph.edges.append(
                                    VisualEdge(
                                        nodes_dict[n],
                                        ...
                                        color))
        if draw_uses:
            logger.info('Looking at %s' % visitor.uses_edges)
            color = "#000000"
