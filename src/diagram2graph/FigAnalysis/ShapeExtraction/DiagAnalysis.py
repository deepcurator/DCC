...
if __name__ == '__main__':
    ...
    if (process.returncode == 0):
        print("[INFO] loading images ...")
        print(glob.glob(os.path.join(op_path_all, '*png')))
        ...
        for filename in glob.glob(os.path.join(op_path_all, '*png')):
            ...
            if (filename.find('Figure') != -1): 
                ...
                print(figTypeResult)
                print(figTypeDiag)
                if ~figTypeResult and figTypeDiag:
                    im, thresh_im, gray_imcv = preprocessImage(filename, 0)
                    binType, mcType = figtypedetector.detectFigType(im)
                
        #--------------------------------------------------------------------------------------------
                    if mcType < 3:
                        opname = os.path.join(os.path.dirname(abspath), "SelectedImage/" + image_file_name + ".jpg")
                        print(opname)
                        cv2.imwrite(opname, im)                
