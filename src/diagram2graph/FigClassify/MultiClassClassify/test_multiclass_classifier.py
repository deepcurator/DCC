...
# Command: python test_multiclass_classifier.py --model vgg16 --outputdir <dir where computed features are models are saved>
...
ap.add_argument("-o", "--outputdir", required=True, help='Path to dir containing MultiClassFeatures and BinaryModels')
...
testFeature_path = os.path.join(feature_dir_path, args["model"]+'multiclass_test_features.h5')
testLabel_path = os.path.join(feature_dir_path, args["model"]+'multiclass_test_labels.h5')
...
if (os.path.isfile(model_json_path) and os.path.isfile(testFeature_path)):
	...
	testfeatures_string = h5f_testdata[args["model"]+'multiclass_test_features']
	testlabels_string   = h5f_testlabel[args["model"]+'multiclass_test_labels']
